...
source('prototype/method_quantiyContext.R')
...
for (group in groups) {
  ...
  for (type in c('train','test')){
    ...
    for (binnedTokens in bins) {
      ...
      metrics <- quantifyContext(filepath,datafile_name,target_corpus,dsmProj,most_freq_words,minMatches=25,window_length=15,sim_count=1000)
