    ...
    for (group in groups) {
      ...
      for (type in c('test')){
        ...
        for (i in seq(1,nrow(indices))){
          ...
          if (debug) topWords <- topWords[1]
          write.csv(topWords, paste0('./data_dsicap/',group,'/RData/','targetWords_',type,i,'.csv'))
  ...
  if(sentiment == T){
    ...
    for (group in groups) {
      ...
      for (type in c('train','test')){
        ...
        bins <- dataFiles[str_detect(dataFiles,paste0('processedTokens_',type,"[0-9]+\\.RData"))]
  ...
  if(createDSM == T){
    for (group in groups) {
      ...
      for (type in c('train','test')){
        dataFiles <- list.files(paste0('./data_dsicap/',group,"/RData"))
        print(dataFiles)
        bins <- dataFiles[str_detect(dataFiles,paste0('wordCo_',type,"[0-9]+\\.RData"))]
        print(bins)
